...
import {getReadableTextColor} from '@/config/helpers/brightness-color';
...
export default function PokemonScreen({navigation, route}: Props) {
  ...
  if (!pokemon) {
    return <Loader size={100} color="black" style={{backgroundColor: color}} />;
  }
      ...
      <View style={{flexDirection: 'row', marginHorizontal: 20, marginTop: 10}}>
        {pokemon.types.map(type => (
          <Chip
            key={type}
            mode="outlined"
            selectedColor={isDark ? color : getReadableTextColor(color)}
            style={{marginLeft: 10}}>
